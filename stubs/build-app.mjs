...
async function stripPhpFilesParallel(files) {
  const phpFiles = files.filter(
    f => !f.isDir && f.path.endsWith('.php') && !f.path.endsWith('.blade.php') && !f.path.startsWith('php-stubs') && !f.path.startsWith('storage/framework/views/'),
  );
...
function createTar(files, { stripWhitespace = false, strippedContents = new Map() } = {}) {
  ...
  for (const file of files) {
    if (file.isDir) {
      ...
      if (stripWhitespace && file.path.endsWith('.php') && !file.path.endsWith('.blade.php') && !file.path.startsWith('php-stubs') && !file.path.startsWith('storage/framework/views/')) {
