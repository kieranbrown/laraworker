...
Work on this task until it is complete, tested, and verified. If you are given a solution that is wrong during verification, find a better one.
...
- `fuel run` / `fuel run:stop [name|run-id]` / `fuel run:status` / `fuel run:logs` - Manage dev processes defined in .fuel/run.yml
