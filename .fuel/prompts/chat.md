...
You MUST run 'fuel epic:add' (check the fuel create plan skill or 'fuel --help' for usage) as soon as possible once you've decided to build something across multiple tasks, so the user can follow the live updates in the epic sidebar on the right of their screen, tell the user the epic id immediately.

Add the basic epic as soon as possible, then investigate and update it as you go.
...
- Create fuel tasks/epics for all actionable implementation work (code/docs/config changes, execution, or follow-up deliverables).

== FUEL COMMAND USAGE ==

**Command discovery:** Run `fuel --help` for all commands, `fuel <command> --help` for detailed help.

**Adding tasks:** ALWAYS set `--complexity`:
```bash
fuel add "Title" -d "Simple description" --complexity=simple
fuel add "Title" --complexity=moderate < desc.txt  # Multi-line or special chars
```
Complexity: `trivial` (typos) | `simple` (single focus) | `moderate` (multiple files) | `complex` (requires judgement/coordination)

**Task description quoting hazard:**
⚠️  NEVER use `-d`/`--description` for descriptions with `$`, backticks, quotes, or newlines — shell will interpolate/mangle them.
For anything beyond plain text: write to file, pipe via stdin (`< desc.txt`).

**Cross-project work:** If work requires action outside this repo (another service, external system, credentials), add task with `--labels=needs-human` so human reviews it.
...
- When planning epics after running `fuel epic:add`, use `fuel epic:plan <e-id>` to update the plan file (pipe content via stdin)
