...
export class PhpCgiBase {
  ...
  handleFetchEvent(event) {
    ...
    const { files: sharedLibFiles, urlLibs: sharedLibUrls } = resolveDependencies(
      this.sharedLibs,
      this,
    );
    ...
    if (globalThis.location) {
      ...
      const staticUrls = [
        ...
        ...libFiles.map((file) => file.url),
        ...Object.values(libUrls),
      ]
      ...
      isWhitelisted =
        url.pathname.substr(0, prefix.length) === prefix && url.hostname === self.location.hostname;
      isBlacklisted =
        url.pathname.match(/\.wasm$/i) ||
        staticUrls.includes(url.pathname) ||
        this.exclude.findIndex((exclude) => url.pathname.substr(0, exclude.length) === exclude) >
          -1 ||
        false;
    } else {
  ...
  refresh() {
    ...
    return (this.binary = this.binLoader
      .then(({ default: PHP }) => new PHP(phpArgs))
      .then(async (php) => {
  ...
  async request(request) {
    const { url, method = "GET", get, post, contentType } = await breakoutRequest(request);
    ...
    try {
      ...
      return await navigator.locks.request("php-wasm-request-lock", async () => {
        ...
        exitCode = await php.ccall("main", "number", ["number", "string"], [], { async: true });
  ...
  async storeInit() {
    ...
    await this.writeFile("/config/init.json", JSON.stringify({ settings, env }, null, 4), {
      encoding: "utf8",
    });
  }
